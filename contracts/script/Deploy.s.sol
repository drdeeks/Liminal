...
contract Deploy is Script {
    ...
    address constant MONAD_MAINNET_PRICE_FEED = address(0); // TBD when mainnet launches
    address constant MONAD_TESTNET_PRICE_FEED = 0x0c76859E85727683Eeba0C70Bc2e0F5781337818;
    ...
    function run() external {
        ...
        require(
            ...
            "Unsupported chain. Must be Monad (143/10143), Base (8453), or Base Sepolia (84532)"
        );
    ...
    function getNetworkName() internal view returns (string memory) {
        if (block.chainid == MONAD_MAINNET_CHAIN_ID) return "Monad Mainnet";
    ...
    function getPriceFeed() internal view returns (address) {
        if (block.chainid == MONAD_MAINNET_CHAIN_ID) return MONAD_MAINNET_PRICE_FEED;
        if (block.chainid == MONAD_TESTNET_CHAIN_ID) return MONAD_TESTNET_PRICE_FEED;
